...
from datasets.ISIC import get_isic_loader
sys.path.append('..')
...
if __name__ == '__main__':
    # 读取图像
    train_dataloader, test_dataloader = get_isic_loader(batch_size=4)
    # # 读取模型
    chkpt_dir = r'C:\Users\xcm\Desktop\权重\ISIC\ISIC+pre+contr+conv+ms\ISIC_SwinUnet_30k_224x224_ms_dw_0.1_0.0003\model\model_0.8619.pth'
    ...
    for i, (img, label_true) in enumerate(test_dataloader):
        ...
        if i == 0:
            ...
            for j in range(4):
                ...
                plt.imshow(image, cmap='gray')
                plt.imshow(label_to_img(label_pred[j]), alpha=0.6, cmap='viridis') 
                plt.title("pred", fontsize=16)
                plt.axis('off')

                plt.subplot(4, 5, 5+5*j)
                plt.imshow(image, cmap='gray')
                plt.imshow(label_to_img(label_true[j]), alpha=0.6, cmap='viridis') 
                plt.title("true", fontsize=16)
                plt.axis('off')
            plt.show()
            fig.savefig("img/isic_paper_0.1_86.19.png")
            break
