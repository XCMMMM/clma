...
def prepare_model(chkpt_dir_):
    # build model
    model = contr_mae_vit_base_patch16(in_channels=3,mask_ratio=0.75) 
    # load model
...
def run_one_image(img1, img2, model):
    ...
    plt.show()
    fig.savefig("img/mask_ISIC_final.png")
...
if __name__ == '__main__':
    ...
    for img1, label_true1, img2, label_true2 in data_loader:
        ...
        print(img2.shape)
        # 读取模型
        chkpt_dir = r'C:\Users\xcm\Desktop\checkpoint\ISIC\ISIC+pre+contr+conv+ms\ISIC_SwinMAE_30k_224x224_ms_pr\model\supervise_model.pth'
