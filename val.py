...
def test_lidc(model, test_loader, args, cur_itrs, name="test"):
    ...
    with torch.no_grad():
        for i, (img, label_true) in enumerate(tqdm(test_loader)):
            ...
            d, h, j, a = calculate_metric_percase(label_pred.data.cpu().numpy() == 1, label_true.data.cpu().numpy() == 1)
            dice += d*img.shape[0]
            hd95 += h*img.shape[0]
            jaccard += j*img.shape[0]
            asd += a*img.shape[0]
            if i == 0:
    ...
    jaccard /= len(test_loader.dataset)
    asd /= len(test_loader.dataset)
...
def test_acdc(model, test_loader, args, cur_itrs, name="test"):
    ...
    for i_batch, sampled_batch in enumerate(test_loader):
        ...
        if i_batch == 0:
            ...
            img = torch.from_numpy(slice).unsqueeze(0).unsqueeze(0).float().cuda(device)
    ...
    jaccard = np.mean(metric_list, axis=0)[2]
    asd = np.mean(metric_list, axis=0)[3]
    return performance2, mean_hd952, jaccard, asd
...
def test_single_volume(image, label, net, classes, patch_size=[256, 256]):
    ...
    for ind in range(image.shape[0]):
        ...
        device = torch.device("cuda:1")
        input = torch.from_numpy(slice).unsqueeze(0).unsqueeze(0).float().cuda(device)
        net.eval()
        with torch.no_grad():
