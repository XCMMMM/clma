...
class SwinMAE(nn.Module):
    ...
    # 主要目的是将输入张量 x 从分块的形式还原成图像的形式
    def unpatchify(self, x):
        """
        x: (N, L, patch_size**2 *3)
        imgs: (N, 3, H, W)
        """
        # patch_size 是每个图像块的大小
        p = self.patch_size
        h = w = int(x.shape[1] ** .5)
    ...
    def window_masking(self, x: torch.Tensor, r: int = 4,
        ...
        # B--batch_size   L--(HxW)  D--channel
        B, L, D = x.shape
        assert int(L ** 0.5 / r) == L ** 0.5 / r
        # d--每个窗口包含r × r个token，共划分d × d个窗口
        d = int(L ** 0.5 // r)
        # 获取keep令牌的稀疏索引
        ...
        # 通过以下方式获得保留令牌的索引，y=[x/d]*d*r**2+(x mod d)*r
        ...
        # index_shuffle，其中包含了要被保留和要被移除的索引
        index_shuffle = torch.cat([index_keep, index_mask], dim=1)
        # index_restore 数组，该数组用于恢复原始的索引顺序
        ...
        # remove: 一个布尔值，指示是否应该移除掩码的图像块。
        # 如果为 True，则返回一个经过掩码的张量 x_masked 以及相应的掩码 mask，并且还会返回一个用于恢复原始顺序的索引 sparse_restore。
        # 如果为 False，则返回一个经过掩码的张量 x_masked 和相应的掩码 mask，但不会返回索引 sparse_restore。默认值是 False。
        if remove:
    ...
    def forward_encoder(self, x):
        # 将输入的图像分割成一系列固定大小的图像块（或称为“patch”）
        x = self.patch_embed(x)
        # 窗口掩码:遮蔽输入的特定部分，通常是为了让模型关注输入的特定区域
...
class ContrastiveMAEViT(SwinMAE):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, mask_ratio=0.75, 
                decoder_embed_dim=768, depths=(2, 2, 2, 2), embed_dim=96, num_heads=(3, 6, 12, 24),
                window_size=7, qkv_bias=True, mlp_ratio=4,
                drop_path_rate=0.1, drop_rate=0, attn_drop_rate=0,
                norm_layer=partial(nn.LayerNorm, eps=1e-6), use_proj=False):
        super().__init__(img_size=img_size, patch_size=patch_size, in_chans=in_chans, mask_ratio=mask_ratio, 
                        decoder_embed_dim=decoder_embed_dim, depths=depths, embed_dim=embed_dim, num_heads=num_heads,
        ...
        if use_proj:
            self.projection_head = nn.Sequential(nn.Linear(self.decoder_embed_dim, self.decoder_embed_dim, bias=False),
                                                 nn.BatchNorm1d(self.decoder_embed_dim),
                                                 nn.ReLU(inplace=True),  # first layer
                                                 nn.Linear(self.decoder_embed_dim, self.decoder_embed_dim, bias=False),
                                                 nn.BatchNorm1d(self.decoder_embed_dim),
                                                 nn.ReLU(inplace=True),  # second layer
                                                 nn.Linear(self.decoder_embed_dim, self.decoder_embed_dim, bias=False),
                                                 nn.BatchNorm1d(self.decoder_embed_dim, affine=False))  # output layer
        self.predictor = nn.Sequential(
            nn.Linear(self.decoder_embed_dim, self.decoder_embed_dim, bias=False),
            nn.BatchNorm1d(self.decoder_embed_dim),
            nn.ReLU(inplace=True),  # hidden layer
            nn.Linear(self.decoder_embed_dim, self.decoder_embed_dim)  # output layer
        )

    def forward(self, view1, view2): 
        # First call the forward method of the autoencoder
        latent1, mask1 = super(ContrastiveMAEViT, self).forward_encoder(view1)
        pred1 = super(ContrastiveMAEViT, self).forward_decoder(latent1)  # [N, L, p*p*3]
        loss1 = super(ContrastiveMAEViT, self).forward_loss(view1, pred1, mask1)
        # Call the encoder for the second view
        latent2, mask2 = self.forward_encoder(view2)
        pred2 = self.forward_decoder(latent2)  # [N, L, p*p*3]
        loss2 = self.forward_loss(view2, pred2, mask2)
        loss = 0.5 * (loss1 + loss2)
        
        # Now, we perform the contrastive part on the embeddings
        # First we reshape the tensors so that they can be handled easily
        latent1 = latent1.view(-1, latent1.shape[3])
        latent2 = latent2.view(-1, latent2.shape[3])

        p1 = self.predictor(latent1)
        p2 = self.predictor(latent2)
        
        pred1 = self.unpatchify(pred1)
        mask1 = mask1.unsqueeze(-1).repeat(1, 1, self.patch_embed.patch_size ** 2 * self.in_chans)  # (N, H*W, p*p*3)
        mask1 = self.unpatchify(mask1)  # 1 is removing, 0 is keeping
        
        pred2 = self.unpatchify(pred2)
        mask2 = mask2.unsqueeze(-1).repeat(1, 1, self.patch_embed.patch_size ** 2 * self.in_chans)  # (N, H*W, p*p*3)
        mask2 = self.unpatchify(mask2)  # 1 is removing, 0 is keeping
        return loss, pred1, mask1, pred2, mask2, p1, p2, latent1.detach(), latent2.detach()



def swin_mae(in_channels=3, mask_ratio=0.75, **kwargs):
    model = SwinMAE(
        img_size=224, patch_size=4, in_chans=in_channels,
        mask_ratio=mask_ratio, decoder_embed_dim=768,
...
def contr_mae_vit_base_patch16(in_channels=3, mask_ratio=0.75, **kwargs):
